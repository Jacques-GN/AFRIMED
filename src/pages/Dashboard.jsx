...
const RACCOURCIS = [
  {
    ...
    description: 'Créer un dossier et générer un code unique',
    chemin: '/nouveau-patient',
    icone: UserPlus,
    accent: 'from-blue-500/10 to-blue-500/0 text-blue-600',
  },
  {
    label: 'Patients',
    description: 'Rechercher un dossier existant',
    chemin: '/patients',
    icone: Users,
    accent: 'from-green-500/10 to-green-500/0 text-green-600',
  },
  {
    label: 'Rendez-vous',
    description: 'Suivis programmés',
    chemin: '/rendez-vous',
    icone: Calendar,
        ...
        {RACCOURCIS.map((r) => {
          ...
          return (
            <Link
              key={r.chemin}
              to={r.chemin}
              className={`group relative overflow-hidden rounded-2xl bg-white border border-slate-200 p-5 shadow-soft transition-lift`}
            >
              ...
              <div className="relative">
                ...
                <ArrowRight
                  size={16}
                  className="absolute top-0 right-0 text-slate-300 group-hover:text-slate-500 group-hover:translate-x-0.5 transition-all"
                />
              </div>
            </Link>
          )
